...
2. **Implementation:**
   ...
   **Solution**
   Part 1:
   1.A database engine is the software that a database managment system uses to create, read, update, and delete data from the database.
   2.The query processor is the component of the database system that processes SQL requests.
   3.Storage Manager is the program that provides an interface between the data stored in the database and the queries received from the query processor.
   4.It is The process that oversees, coordinates and executes any given attempted transaction.

   Part 2:
   1.The Select operation is used to select rows from the table on a condition mentioned in the syntax of selection.
   2.The Projection selects colaums of the mentiond conditions and does not repeat the same value twice(Only gives the colaums of the mentioned condition and not the full row).
   3.The Union operation is used ot combine the results of of two select operations.
   4.for example if we had table A and B, The Set difference operation(A-B) would include all the tuples found in A but not in B.
   5.for example if we had table A and B, The Cartesian Product would apply 

   Part 3:

   screenshot can be found in the Practicals solutions.
