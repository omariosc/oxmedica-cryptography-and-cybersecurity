...
2. **Examples:**
   - Provide examples of how these operations can be used to query a database.
    Selection: Suppose we have a relation of about 500 people in a city of all ages. We can use the selection operation to get only the people who are over 70 years old
    Projection: In this town we can use the projection operation to get specific date and elimanate all else, so we use it to get only the people that have the name mohammed and are 60 years old.
    Union: We can use union to combine to tuples that have the same thing while removing duplicates, for example we want people who live in a specific part of town and people who are all in the same book club while removing duplicates.
    Set Difference:
